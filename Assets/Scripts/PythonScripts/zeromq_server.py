...
def build_standard_command(model, prompt, new_dir, newDir, inputFilePath, output_dir, bvh2fbxConvertCommand, iterations, motion_length=1):
    if model == "GMD":
        return f"conda activate gmd && python -m sample.generate --model_path ./save/unet_adazero_xl_x0_abs_proj10_fp16_clipwd_224/model000500000.pt --output_dir {new_dir} --text_prompt \"{prompt}\" --motion_length {motion_length} && python .\\smpl2bvh.py --prompt \"{prompt}\" --input_file {inputFilePath} --output_dir {output_dir}\\{newDir} --iterations {iterations} && conda activate bvh2fbx && {bvh2fbxConvertCommand}{output_dir}\\{newDir}\\{newDir}_0.bvh && {bvh2fbxConvertCommand}{output_dir}\\{newDir}\\{newDir}_1.bvh && {bvh2fbxConvertCommand}{output_dir}\\{newDir}\\{newDir}_2.bvh"
    elif model == "MDM":
        return f"conda activate mdm && python -m sample.generate --model_path ./save/humanml_enc_512_50steps/model000750000.pt --text_prompt \"{prompt}\" --output_dir {new_dir} --motion_length {motion_length} && python .\\smpl2bvh.py --prompt \"{prompt}\" --input_file {inputFilePath} --output_dir {output_dir}\\{newDir} --iterations {iterations} && conda activate bvh2fbx && {bvh2fbxConvertCommand}{output_dir}\\{newDir}\\{newDir}_0.bvh && {bvh2fbxConvertCommand}{output_dir}\\{newDir}\\{newDir}_1.bvh && {bvh2fbxConvertCommand}{output_dir}\\{newDir}\\{newDir}_2.bvh"
    elif model == "OmniControl":
        return f"conda activate omnicontrol && python -m sample.generate --model_path ./save/omnicontrol_ckpt/model_humanml3d.pt --output_dir {new_dir} --num_repetitions 1 --text_prompt \"{prompt}\" --motion_length {motion_length} && python .\\smpl2bvh.py --prompt \"{prompt}\" --input_file {inputFilePath} --output_dir {output_dir}\\{newDir} --iterations {iterations} && conda activate bvh2fbx && {bvh2fbxConvertCommand}{output_dir}\\{newDir}\\{newDir}_0.bvh"
    elif model == "MoMask":
